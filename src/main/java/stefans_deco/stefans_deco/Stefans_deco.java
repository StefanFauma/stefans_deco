...
public class Stefans_deco {
    ...
    // Mein Code:
    // Block
    public static final DeferredBlock<Block> SMALL_STONE = BLOCKS.registerSimpleBlock(
            "small_stone",
            BlockBehaviour.Properties.of().strength(1.5f, 6.0f) // Block hat Stärke 2 (wie Holz)
    );

    // Blockitem
    public static final DeferredItem<BlockItem> SMALL_STONE_ITEM =
            ITEMS.registerSimpleBlockItem("small_stone", SMALL_STONE);
    ...
    public Stefans_deco(IEventBus modEventBus, ModContainer modContainer) {
        ...
        // Note that this is necessary if and only if we want *this* class (Stefans_deco) to respond directly to events.
    ...
    private void addCreative(BuildCreativeModeTabContentsEvent event) {
        if (event.getTabKey() == CreativeModeTabs.BUILDING_BLOCKS) event.accept(SMALL_STONE_ITEM);
    }
